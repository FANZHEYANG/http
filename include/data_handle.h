...
// 处理子对象 2
//void handleSource1SubObj2(const commonFrame& frame,uint32_t& smoke,uint32_t& hat) {
    //if (frame.framedata.hasData == 1) {
        //switch (frame.framedata.data1) {
            ...
            //default: {hat=0;smoke=0; break;}// 未知值不做处理
        //}
   // } else {
      //  hat = 9999;
      //  smoke = 9999;
...
// 处理子对象 5 (吸烟检测) - 只有1个类别
void handleSource1SubObj5(const commonFrame& frame,uint32_t& smoke) {
    // 吸烟检测：hasData=1表示检测到吸烟，hasData=0表示没有吸烟
    smoke = frame.framedata.hasData;
}

// 处理子对象 2 (安全帽检测) - 有2个类别
void handleSource1SubObj2_hat(const commonFrame& frame,uint32_t& hat) {
    // 安全帽检测：data1表示类别
    // data1=0: 不带安全帽
    // data1=1: 带安全帽
    std::cout << "安全帽检测数据 - hasData: " << frame.framedata.hasData 
              << ", data1: " << frame.framedata.data1 
              << ", data2: " << frame.framedata.data2 << std::endl;
    
    if (frame.framedata.hasData == 1) {
        hat = frame.framedata.data1; // 使用data1来区分带/不带安全帽
    } else {
        hat = 0; // 默认不带安全帽
    }
    std::cout << "处理后的安全帽状态: " << hat << std::endl;
}
...
void handleSource1(const commonFrame& frame,Vision_data& vision) {
    switch (frame.frameHead.subObj) {
        ...
        case 2: handleSource1SubObj2_hat(frame,vision.hat); break; // 专门处理安全帽
        case 3: handleSource1SubObj3(frame,vision.inf); break; 
        case 4: handleSource1SubObj4(frame,vision.flame);// 处理子对象 4
        case 5: handleSource1SubObj5(frame,vision.smoke); break; // 处理子对象 5 (吸烟检测)
