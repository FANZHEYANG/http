...
//video path
//std::string ="udpsrc address=230.1.1.1 port=1720 multicast-iface=enP4p65s0 ! application/x-rtp, media=video, encoding-name=H264 ! rtph264depay ! h264parse ! avdec_h264 ! videoconvert ! video/x-raw,width=1280,height=720,format=BGR ! appsink drop=1";
struct pathpoint revPothPoints;
std::atomic<bool> dataProcessed(false); // 原子变量确保线程安全
// 定义全局变量存储数据
std::atomic<bool> pgmdataprocess(false);
std::atomic<bool> video_stopflags(false);
...
commonFrame begin_data_send={
    {0,0,1,6},{0,0,0,0.0f,0.0f}
};
...
void setupSSEConnection(const std::string& topics_str, httplib::Response& res,int udp_socket) {
    ...
    // 初始化回调函数
    auto init_callback = [client_id, topics, &res, client_info, initialized,udp_socket]() {
        // 使用原子标志确保只初始化一次
        ...
        for (const auto& topic : topics) {
            ...
            client_info->topics.insert(topic);
             if(topic=="vision")
                        {
                            sendCommand(udp_socket, begin_data_send, "127.0.0.1", 8870);
                        }
            std::cout << "[" << getCurrentTime() << "] 客户端 " << client_id 
                      << " 订阅了主题 '" << topic 
    ...
    res.set_chunked_content_provider(
        ...
        [client_id, init_callback, close_callback, client_info, initialized](size_t offset, httplib::DataSink& sink) {
            ...
            time_t now = time(nullptr);
            if (now - last_heartbeat >= 30 && client_info->alive.load()) {
...
void pushRobotStatus() {    
    try {
        ...
        json data;
        switch(robot_status.now_point.keyPointType)
        {
            case 0:keyPointType="普通巡检点";
            data = {
            {"运行信息更新"},
            {"工作状态", Workstatus},
            {"当前巡检位置"},
          ...
          //  {"任务结束时间", robot_status.task_end_time}
            ...
            case 2:keyPointType="充电点";
            video_stopflags.store(true);
            data = {
            {"运行信息更新"},
            {"巡检完成",getCurrentTime()},
            {"已完成巡检点数",10}
        };break;
            case 3:keyPointType="待梯点";
            data = {
            {"运行信息更新"},
            {"工作状态", Workstatus},
            {"当前巡检位置"},
...
void trigger(const commonFrame& frame) {
    if (frame.frameHead.frameType == 1) {
        switch (frame.frameHead.source) {
            case 1: //handleSource1(frame,processdata.vision); 
...
public:
    ...
    bool openCamera() {
        ...
        if (!camera_.isOpened()) {
            // 使用GStreamer管道接收网络视频流
            const string pipeline = 
                "udpsrc address=230.1.1.1 port=1720 multicast-iface=enP4p65s0 ! "
                "application/x-rtp, media=video, encoding-name=H264 ! "
                "rtph264depay ! h264parse ! avdec_h264 ! "
                "videoconvert ! video/x-raw,width=1280,height=720,format=BGR ! "
                "appsink drop=1";
            
            camera_.open(pipeline, CAP_GSTREAMER);
            
            if (camera_.isOpened()) {
                // 获取实际帧率
                fps_ = camera_.get(CAP_PROP_FPS);
                if (fps_ <= 0) fps_ = 30.0; // 默认30fps
                cout << "成功连接到视频流，帧率: " << fps_ << "fps" << endl;
            } else {
                cerr << "无法打开GStreamer管道！请检查网络和参数。" << endl;
            }
        }
    ...
    bool readFrame(Mat& frame) {
        ...
        // 如果读取失败，尝试重置连接
        if (!success) {
            cerr << "读取帧失败，尝试重新连接..." << endl;
            camera_.release();
            openCamera(); // 尝试重新打开
            if (camera_.isOpened()) {
                success = camera_.read(frame);
            }
        }
...
void start_http_server(int udp_socket){
   ...
   svr.Get("/sse", [&](const Request& req, Response& res) {
    ...
    svr.Get("/smoke_concentration", [&](const Request& req, Response& res) {
        sendCommand(udp_socket, request_yanwu1Frame, "127.0.0.1", 8083);
        sendCommand(udp_socket, request_yanwu2Frame, "127.0.0.1", 8083);
        while (!dataProcessed) {
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
        }
        json response_data = {
...
// 添加时间戳补偿机制
    ...
    svr.Post("/robot", [&](const Request& req, Response& res) {
       ...
       svr.Post("/robot/temp", [&](const Request& req, Response& res) {
    auto jsonData = json::parse(req.body);
    std::string action = jsonData["action"]; // 可能的值是 "open" 或 "close"
    ...
    // 根据操作填充 data1
    if (action == "open") {
    	sendCommand(udp_socket, close_infframe, "127.0.0.1", 8870);
    	std::this_thread::sleep_for(std::chrono::milliseconds(2000));
        frame.framedata.data1 = static_cast<uint32_t>(1); // OPEN
    } else if (action == "close") {
        frame.framedata.data1 = static_cast<uint32_t>(2); // CLOSE
    ...
    // 发送控制指令
    sendCommand(udp_socket, frame, "127.0.0.1", 9010);
//    sendCommand(udp_sockfd, frame, "192.168.110.123", 8870);
    res.set_content("Command executed: " + action + " " , "text/plain");
});
    svr.Post("/scheduled_task", [&](const Request& req, Response& res) {
     ...
     svr.Get("/video_intime", [](const httplib::Request &, httplib::Response &res) {
        auto& camManager = CameraManager::getInstance();
        if (!camManager.openCamera()) {
            res.status = 500;
            res.set_content("Failed to open video stream", "text/plain");
            return;
        }
        
        const cv::Size target_size(1280, 720); // 输出分辨率
        int frame_counter = 0;
        double fps = camManager.getFPS();
        
        res.set_chunked_content_provider(
            "multipart/x-mixed-replace; boundary=frame",
            [target_size, fps, frame_counter](size_t, httplib::DataSink &sink) mutable {
                cv::Mat frame;
                if (!CameraManager::getInstance().readFrame(frame)) {
                    return false;
                }
                
                // 调整帧大小（从1280x720缩放到640x480）
                if (frame.size() != target_size) {
                    cv::resize(frame, frame, target_size, 0, 0, INTER_AREA);
                }
                ...
                // 写入响应
                if (!sink.write(chunk_header.data(), chunk_header.size()) ||
                    !sink.write(reinterpret_cast<char*>(buffer.data()), buffer.size()) ||
                    !sink.write("\r\n", 2)) {
                    return false;
                }
                ...
                // 确保补偿值非负
                int compensation = std::max(0, delay_ms - static_cast<int>(elapsed.count()));
                std::this_thread::sleep_for(std::chrono::milliseconds(compensation));
    ...
    svr.Get("/video_stop", [&](const httplib::Request &, httplib::Response &res) {  
            video_stopflags.store(true);
             res.set_content("stop camera", "text/plain");
    });
    ...
    // 打开摄像头（明确指定 V4L2 后端）
     auto& camManager = CameraManager::getInstance();
        if (!camManager.openCamera()) {
            res.status = 500;
            res.set_content("Failed to open camera", "text/plain");
            return;
        }
    VideoWriter video("output.avi", VideoWriter::fourcc('M', 'J', 'P', 'G'), 30, Size(1280, 720));
	const int frameDelay = 1000 / 30;  // 30fps对应的帧间隔
   ...
   // destroyAllWindows();
    ...
    res.set_content("视频已保存为: output.avi", "text/plain");
});
